# =============================================================================
# 5_generate_spectrograms.py
#
# This is the main, production-ready script for generating spectrograms for
# the entire dataset. It uses the validated and optimized logic from our tests.
#
# Key Features:
# - Processes all cases from the final cohort.
...
TRAIN_IDS_FILE = '../data/processed/preop_train_cleaned.csv'
TEST_IDS_FILE = '../data/processed/preop_test_cleaned.csv'
OUTPUT_DIR = '../data/processed/'
SPECTROGRAM_SUBDIR = 'spectrograms/pleth'
LOG_FILE_PATH = 'spectrogram_generation.log' # Log file in the same directory as the script
...
def main():
    ...
    setup_logging()
    logging.info("--- Phase 5: Scalable Waveform Data Generation Pipeline ---")
    ...
    try:
        ...
        try:
            ...
            results_df = pd.DataFrame(all_results, columns=['image', 'aki_label', 'caseid'])
            ...
            final_train_manifest = os.path.join(OUTPUT_DIR, 'train_data.csv')
            final_test_manifest = os.path.join(OUTPUT_DIR, 'test_data.csv')
