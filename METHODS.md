...
To complement the tree-based models, we added interpretable **ExplainableBoostingClassifier** runs using the `interpret` library. EBM training fixes several stability-oriented defaults (0 pairwise interactions, `missing="gain"`, 20 inner bags, 14 outer bags, 1024 histogram bins, multi-core training, seeded randomness) while Optuna tunes `max_leaves`, `smoothing_rounds`, `learning_rate`, `validation_size`, `early_stopping_rounds`, `early_stopping_tolerance`, `min_samples_leaf`, `min_hessian`, and `greedy_ratio` via 5-fold stratified CV to maximize AUPRC. The same out-of-fold logistic recalibration and Youden-J threshold selection used for XGBoost are applied without modification, and EBM predictions are reported alongside the XGBoost branches in consolidated tables and figures. When `step_07_train_evaluate.py` is invoked with `--export_ebm_explanations`, the resulting calibrated test predictions are paired with global and local explanation payloads from `interpret`, interaction rankings from the learned terms, and reconciliation tables linking each test case ID to raw logits, calibrated probabilities, predicted labels, and per-term contributions. These artifacts are written under `results/models/ebm/<outcome>/<branch>/<feature_set>/artifacts/ebm_xai/` with companion bar plots (Matplotlib and Plotly/Kaleido) and a README describing the logistic recalibration parameters applied to map logits to probabilities.
...
*   **Interpretability**: SHapley Additive exPlanations (**SHAP**) values were calculated for XGBoost models to quantify feature contributions. EBM runs rely on the model's native additive terms; SHAP is skipped, but optional EBM explainability exports combine global/local `interpret` payloads with calibrated probabilities and per-case attribution tables to keep explanations aligned with deployed thresholds.
